...
def get_metrics():
    ...
    for r in root_runs:
        ...
        is_escalated = False
        # In this pipeline, escalation often means ending at human_review_node
        # We can check child runs or state if available, but usually pa_decision is empty
        ...
        # Capture Failed Five
        # Patient ID is in r.inputs['patient_data']['id'] likely
        p_id = "UNKNOWN"
        try:
...
if __name__ == "__main__":
    ...
    for pid, data in failed_five.items():
        print(f"--- {pid} ---")
        print(f"Decision: {data['actual_decision']}")
        print(f"Rules Checker Output:\n{data['rules_checker_output']}\n")
